...
class WithdrawRepository(private val context: Context,
    ...
    override fun withdraw(symbol: String, accountAddress: String, password: String, targetAddress: String, amount: Double): Single<TransactionModel> {
        return Single.create { subscriber ->
            ...
            context.retrofit.withdrawAPI.withdraw(symbol, walletData.walletFileName, walletData.walletData, password, targetAddress, amount.toString()).subscribe({ transactionResult ->
                if (transactionResult != null && transactionResult.isStatus) {
